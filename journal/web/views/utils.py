...
def PDF(
    ...
    css_template_path = os.path.join(settings.BASE_DIR, "static/css/report-pdf.css")
    css = CSS(css_template_path, font_config=font_config)
    html = render_to_string(template_html, context)
    html_weasyprint = HTML(string=html)
    
    # Debugging Weasyprint, the log cluttering was because of
    # font.clean(cmap) in font.py from weasyprint.pdf.fonts
